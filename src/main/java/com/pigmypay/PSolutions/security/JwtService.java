...
public class JwtService {
    ...
    // 🚨 In production, move this to application.properties (@Value("${jwt.secret}"))
    ...
    public String generateToken(User user) {
        ...
        // MULTI-TENANT BLAST WALL: The Tenant ID is cryptographically signed into the token
        if (user.getTenant() != null) claims.put("tenantId", user.getTenant().getId());
        if (user.getBranch() != null) claims.put("branchId", user.getBranch().getId());
